...
export const ReceiveTicketConfirmModal = ({
  event,
  ticket,
}: ReceiveTicketConfirmModalProps) => {
    ...
    <Dialog>
      <DialogTrigger asChild>
        <div className="m-auto">
          <Button className="text-white rounded-full px-10 h-[42px] bg-yellow-500 border border-yellow-500 hover:opacity-70 hover:bg-yellow-500">
            チケットを受け取る
          </Button>
        </div>
      </DialogTrigger>
      <DialogContent>
        ...
        <DialogFooter className="flex sm:justify-center mt-4 mb-4">
          {isReceived ? (
            <p className="text-xl text-yellow-500 font-bold text-center">
              チケットを受け取りました
            </p>
          ) : (
