    ...
    <Dialog>
      <DialogTrigger asChild>
        <div className="text-center m-auto">
          <Button className="text-white rounded-full px-10 h-[42px] bg-[#1eb98c] border border-[#1eb98c] hover:opacity-70 hover:bg-[#1eb98c]">
            チケットを表示
          </Button>
        </div>
      </DialogTrigger>
      <DialogContent>
        ...
        <div className="max-w-[560px] w-full bg-[#66efc8] bg-opacity-20 m-auto py-10 px-10 rounded-sm">
          <div className="max-w-[375px] w-full m-auto">
            ...
            <div className="mt-6">
              <span className="block mb-2 text-center font-bold text-lg">
                {username}
              </span>
              <span className="block mb-1 text-center font-bold text-lg text-[#6b7280]">
            ...
            {ticket.used_time ? (
              <div className="mt-10">
                <p className="text-[42px] font-bold text-center text-red-500">
                  入場済み
                </p>
              </div>
            ) : (
              <div className="mt-10">
                <p className="text-lg text-center font-bold text-red-500 mb-4">
                  入場するときにスタッフを見せてください。
                  <br />
                  スタッフが消し込みを行います。
                </p>
                <div className="flex justify-center">
                  <SlidingButton
                    updateUsedState={handleUpdateTicketUsed}
                    ticketId={ticket.id}
                  />
                </div>
              </div>
